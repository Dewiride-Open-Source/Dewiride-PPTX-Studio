...
**PowerPoint authored first.** `tools/ground-truth/author-sheets.ps1` drives the
...
(`tools/ground-truth/sheets.ts`), read back through
`tools/ground-truth/read-sheets.ps1` and reduced by `analyse-sheets.ts`.
