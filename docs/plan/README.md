...
| 3.6        | Anchors, insets, vertical text                                         | ⚠️ done        | [0039](../adr/phase-3-text/0039-upright-glyphs-in-vertical-text.md) — `eaVert` and `mongolianVert` draw, standing the `a:ea` slot upright and turning everything beside it; the insets now turn with the frame, which `vert-ins-vert` had been catching unread — the two WordArt directions stay refused because their per-glyph column is unmeasured, so `a10-rtl-cjk-03` is still a gap |
