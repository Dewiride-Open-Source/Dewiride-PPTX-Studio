...
if (options.manifest) {
  ...
  const manifest = {
    $comment:
      ...
      'tools/corpus/written/build-written.ts --manifest; every description and features map is a ' +
      'literal in decks.ts beside it.',
    manifestVersion: 1,
    collection: 'written',
    generatedIn: '1.1',
    adr: 'docs/adr/0009-the-corpus.md',
    generator: 'tools/corpus/written/build-written.ts',
    ...
    entries: [...built]
      ...
      .map((item) => ({
        ...
        recipe: {
          tool: 'tools/corpus/written/build-written.ts',
