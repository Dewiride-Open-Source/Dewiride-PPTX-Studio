...
import { a46HundredSlides } from './deck/a46-hundred-slides/deck.ts';

/**
 * The Tier A roster, in id order.
 *
 * `tools/corpus/ROSTER.md` is the plan: forty-five decks, what each is the
 * probe for, and which experiment gates the ones that are not yet buildable.
 * This array is what exists, and the two agree: the one slot the roster numbers
 * and this array lacks, `a28-model3d`, is cut rather than pending, and its
 * census key is declared in the manifest's `uncovered` array. A slot named in the roster and missing here
