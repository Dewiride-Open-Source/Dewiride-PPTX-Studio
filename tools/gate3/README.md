...
- the SVG at every zoom is byte-identical to the SVG at 100 % once the root's size, the values of
  ...
  masked — what the stroke rule rounds to device pixels, and the head drawn from that pen (F2, ADR 0054) — and the thumbnail is too, once its id prefix is renamed;
