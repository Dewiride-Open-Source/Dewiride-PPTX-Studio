...
#
#   powershell -File tools/ground-truth/render/transforms/read.ps1 -Dir <work-dir>
#
...
#
# Measured on PowerPoint's own output in `tools/ground-truth/render/transforms/author.ps1`: for a shape
...
if (-not (Test-Path -LiteralPath $inputsPath)) {
    throw "no transform-inputs.json in $root - run tools/ground-truth/render/transforms/build-deck.ts first"
}
