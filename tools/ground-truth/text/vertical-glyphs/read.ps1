# Experiment T10, step 2 - ask PowerPoint how it turned each glyph.
#
#   powershell -File tools/ground-truth/text/vertical-glyphs/read.ps1 -Dir <work-dir>
#
...
# which way the outline actually points. ADR 0039.
#
...
$inputsPath = Join-Path $root 'glyph-inputs.json'
if (-not (Test-Path -LiteralPath $inputsPath)) {
    throw "no glyph-inputs.json in $root - run build-deck.ts first"
}
...
$file = Join-Path $root 'vertical-glyphs.pptx'
$record = [ordered]@{
    file        = 'vertical-glyphs.pptx'
    opened      = $false
    repaired    = $null
    error       = $null
...
[System.IO.File]::WriteAllText(
    (Join-Path $root 'glyph-readings.json'), $json, (New-Object System.Text.UTF8Encoding($false)))

$state = if ($record.opened) { if ($record.repaired) { 'REPAIRED' } else { 'ok' } } else { 'REFUSED' }
Write-Host ("vertical-glyphs {0} {1} slide(s) {2} shape(s)" -f $state, $record.slides.Count, $record.shapes.Count)
