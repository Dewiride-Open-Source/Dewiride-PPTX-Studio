...
import type { Environment } from './raster/fonts.ts';
...
/** What a platform's baseline holds: the font lock it was recorded under, and a digest per key. */
export interface Baseline {
  readonly environment: Environment;
  readonly slides: Readonly<Record<string, { rasterSha256: string; svgSha256: string }>>;
}

/** The baseline at `path`, or null where this platform has never recorded one. */
...
/** The baseline as the file it is committed as: keys sorted, so a diff reads as slides. */
export function baselineJson(baseline: Baseline, tool: string): string {
  return `${JSON.stringify(
    {
      $comment: `GENERATED by ${tool} --record. Do not edit by hand.`,
      environment: baseline.environment,
      slides: Object.fromEntries(
        Object.entries(baseline.slides).sort(([a], [b]) => (a < b ? -1 : a > b ? 1 : 0)),
      ),
    },
    null,
    2,
